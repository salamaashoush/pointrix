...
describe('Pointrix', () => {
  ...
  describe('Constructor', () => {
    it('creates an instance', () => {
      const instance = new Pointrix(el)
      expect(instance).toBeInstanceOf(Pointrix)
      instance.destroy()
    })
    ...
    it('applies default options', () => {
      const onStart = vi.fn()
      const instance = new Pointrix(el, { onStart, threshold: 0 })
  ...
  describe('Pointer tracking', () => {
    ...
    it('tracks pointer move after activation', () => {
      const onMove = vi.fn()
      const instance = new Pointrix(el, { onMove, threshold: 0 })
    ...
    it('tracks pointer up and fires onEnd', () => {
      const onEnd = vi.fn()
      const instance = new Pointrix(el, { onEnd, threshold: 0 })
  ...
  describe('Threshold', () => {
    it('does not start interaction until movement exceeds threshold', () => {
      const onStart = vi.fn()
      const instance = new Pointrix(el, { onStart, threshold: 10 })
    ...
    it('starts interaction once movement exceeds threshold', () => {
      const onStart = vi.fn()
      const instance = new Pointrix(el, { onStart, threshold: 5 })
    ...
    it('uses default threshold of 3 when not specified', () => {
      const onStart = vi.fn()
      const instance = new Pointrix(el, { onStart })
  ...
  describe('Multiple instances on same element', () => {
    it('coordinates via priority system - higher priority wins', () => {
      ...
      const low = new Pointrix(el, { onStart: onStartLow, threshold: 0 })
      const high = new Pointrix(el, { onStart: onStartHigh, threshold: 0 })
    ...
    it('does not remove styles until last instance is destroyed', () => {
      const instance1 = new Pointrix(el)
      const instance2 = new Pointrix(el)
  ...
  describe('Factory function', () => {
    ...
    it('creates instance from CSS selector', () => {
      el.id = 'test-element'
      const instance = pointrix('#test-element')
      expect(instance).toBeInstanceOf(Pointrix)
      instance.destroy()
    })

    it('throws if selector does not match any element', () => {
      expect(() => pointrix('#nonexistent')).toThrow('Element not found')
    })

    it('passes options through', () => {
      const onStart = vi.fn()
      const instance = pointrix(el, { onStart, threshold: 0 })
